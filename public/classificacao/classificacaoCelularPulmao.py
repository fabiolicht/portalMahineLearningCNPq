...
nickname = sys.argv[1]
model = load_model("../../classificacaoCelularPulmao.h5", compile=False)
...
for i in range(qtdImg):
    ...
    if (class_index == 0):
        print("AdenocarcinomaQQQQQQ", end='')
    if (class_index == 1):
        print("Squamous_Cell_CarcinomaQQQQQQ", end='')
    if (class_index == 2):
    ...
    class_names = ['Adenocarcinoma', 'Squamous_Cell_Carcinoma', 'Normal']
