...
class ResultControllerUltrassomMamaVideo extends Controller
{
    public function __invoke(Request $request, string $resultado, string $path)
    {
        ...
        if (strstr($resultado, "Maligno")) {
            ...
            $process = new Process([$executavel, 'segmentacao/segmentacaoUltrassomMamaMalignoVideo.py', $path]);
            ...
            $process = new Process([$executavel, 'segmentacao/segmentacaoUltrassomMamaBenignoVideo.py', $path]);
            $process->start(); // Inicia o processo
            while ($process->isSuccessful())
                ;
            $process->wait(); // Aguarda o processo terminar
